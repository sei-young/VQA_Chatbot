...
from .text import start, regist, pause, usage, image
import telegram
import json
...
def webhook(request):
    ...
    if ((is_json_key_present(telegram_update['message'], 'document')) and (telegram_update['message']['document']['mime_type'] == "image/jpeg" \
        or telegram_update['message']['document']['mime_type'] == "image/png")) or is_json_key_present(telegram_update['message'], 'photo'):
        ...
        try: # 초기 등록 이미지에 대한 예외처리
            ...
            try: # 연속 이미지 등록에 대한 예외 처리 
                ...
                if ((QUESTION.objects.filter(IMAGE_ID=load_image[0]).count()>=7) and (ANSWER.objects.filter(QUESTION_ID=load_question[0]).count()==1)) \
                    ...
                    # 질문 답 7개 이상 등록 후 이미지 등록할 경우 등록진행중이던 이미지 STATUS 1로 변경
                    ...
                    text = '데이터 등록을 시작합니다. 1번째 질문을 입력하세요.'
                    print(fileName)
                    print('이미지 저장 완료')
                ...
                print('Already regist Image : 이미지 연속 등록')
                text = "진행중인 이미지에 대한 1번째 질문을 입력해주세요."
        
        # 처음 사용하는 유저가 이미지 등록할 때
        except IndexError:
            # try:
        ...
        if is_json_key_present(telegram_update['message'], 'text'): # TEXT만 받도록
            print('TEXT')
...
완료 : {2}  중지 : {3}  진행 : {4}'''.format(userName, count_total_image, count_complete_image, count_pause_image, count_ing_image)
                ...
                if received_text[-1] == '?': # 질문 입력받음
                    ...
                    if load_question.count()==0: # 이미지에 대한 질문 유무확인
                        ...
                        text = str(question_cnt) + '번째 질문에 대한 답변을 입력하세요.' 
                    else:
                    ...
                    # except IndexError: # 사용자가 등록한 이미지가 없을 경우, 없어도 되는 부분?
                    #     print('진행중인 사진 없음')
                    #     text = '등록된 사진이 없습니다. 사진을 등록해주세요.'
                    ...
                    try:
                        if ANSWER.objects.filter(QUESTION_ID=load_question[0].QUESTION_ID).count() == 0: # 가장 최근 이미지에 대한 질문의 답을 했는지 확인
                            ...
                            print("답변이 이미 존재")
                            text = str(load_question[0].QUESTION_CNT+1) + '번째 질문을 먼저 입력해주세요.'
                        
                    except IndexError:
                        print("등록된 질문 아예 없음")
                        text = '1번째 질문을 먼저 입력해주세요.'

                    bot.send_message(chat_id=chatID, text=text)  

            except IndexError: # 사용자가 등록한 이미지가 없는 경우
                # text = start
